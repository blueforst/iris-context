...
`;
  ...
  // generatedAt is EXCLUDED from the hash basis so equivalent rebuilds on
  // different days verify as the same generation content (Notion: same frozen
  // source snapshot must produce the same machine authority; the freshness
  // gate tolerates only the date-marker difference).
  const { generatedAt: _generatedAtExcluded, ...hashBasis } = manifest;
  const manifestJson = JSON.stringify(hashBasis, Object.keys(hashBasis).sort());
